...
import math
import commonDef
from aesEncode import MyCrypt as AESCode
# from aesEncode import MyCrypt as AESCode
...
def getResRandomChar(key1 = 33, key2 = 125):
    ...
    elif 33 <= startAscii <= 48:
        return start + getRandomStr(130 + (startAscii - 33) * 50)
    elif 50 <= startAscii <= 90:
        return start + getRandomStr(230 + (startAscii - 50) * 50)
    elif 91 <= startAscii <= 116:
        return start + getRandomStr(420 + (startAscii - 91) * 50)
    elif 117 <= startAscii <= 125:
        return start + getRandomStr(370 + (startAscii - 117) * 50)
    else:
        return start + getRandomStr(350)
...
def EncodeContent_ex(content, spaceNum, specSpace = '', mustType = 0, fileStr = ''):
    KeyList = commonDef.getKeyList1()
    lenList = len(KeyList)

    try:
        keyItem = KeyList[random.randint(0, lenList - 1)]
        if mustType != 0:
            while True:
                if keyItem['type'] == mustType:
                    break
                keyItem = KeyList[random.randint(0, lenList - 1)]
        
        if keyItem['type'] == 2:
            content = aesContent(content, keyItem['key'], keyItem['sign'], spaceNum, specSpace)
        elif keyItem['type'] == 1:
            content = EncodeContent(content, keyItem['key'], keyItem['sign'], spaceNum, specSpace)
        elif keyItem['type'] == 3:
            oldLen = len(content)
            content = zlib.compress(content)
            headStr = b''
            headStr = struct.pack('>H', 0)
            if spaceNum == 0:
                headStr += struct.pack('>H', 0)
            else:
                headStr += struct.pack('>H', 199)
            headStr += struct.pack('>I', 0)
            headStr += struct.pack('>I', oldLen)

            #这个每一个是2个, 需要转换为大端
            # content = ReadFile(fileStr)
            # content = content[4:]
            
            if spaceNum != 0:
                if specSpace == '':
                    space = getResRandomChar()
                    content = keyItem['sign'] + headStr + space  + content
                else:
                    content = keyItem['sign'] + headStr+ specSpace  + content
            else:
                content = keyItem['sign'] + headStr + content

        return content
    except Exception as e:
        print "file error!:" + fileStr
        content = EncodeContent_ex(content, spaceNum, specSpace, 1, fileStr)
        return content
...
def aesContent(content, key, signment, spaceNum, specSpace = ''):
    if content[:len(signment)] == signment:
        return

    AESTool = AESCode(key)
    encrypt = AESTool.myencrypt(content, True)
    if spaceNum != 0:
        if specSpace == '':
            space = getResRandomChar()
            signment = signment + space + encrypt
...
def aesEncode(filePath, key, signment, spaceNum, specSpace = ''):
    content = ReadFile(filePath)
    signment = aesContent(content, key, signment, spaceNum, specSpace = '')
