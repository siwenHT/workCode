#include "LuaSocket.h"
#include "JIGOverheadSpecifiersMG.h"
...
void LuaSocket::onUIThreadReceiveMessage(SocketData* data)
{
	if(data)
	{
		...
		if(data->m_len < 0)
		{
			...
			int tmpSymbolKey[] = {70,43,13,24,59,59,45,59}; 
		...
		if(data->m_flag == CONNECT_RET)
		{
			...
			int tmpSymbolKey[] = {9,45,72,72,43,9,13,51,73,47,47,34,73,9,48}; 
		...
		if(data->m_flag == READ_DATA)
		{
			...
			int tmpSymbolKey[] = {70,43,13,76,38,2,35,58,38,27,73,9,55,43,59}; 
...
{
	JIGOverheadSpecifiersMG jigoverheadspecifiersmg_e;
	jigoverheadspecifiersmg_e.simpleThenProblem(775.11,666,487,866.13);
