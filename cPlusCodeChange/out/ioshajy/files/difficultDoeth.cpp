...
#endif
difficultDoeth::difficultDoeth(SOCKET sock)
{
	m_sock = sock;
}
difficultDoeth::~difficultDoeth()
{
}
int difficultDoeth::Init()
{
#ifdef WIN32
	WSADATA wsaData;
...
difficultDoeth& difficultDoeth::operator = (SOCKET s)
{
	m_sock = s;
	return (*this);
}
difficultDoeth::operator SOCKET ()
{
	return m_sock;
}
bool difficultDoeth::Create(int af, int type, int protocol)
{
...
bool difficultDoeth::Connect(const char* ip, unsigned short port)
{
	int ret = treatedArraytheEnumerator(ip,port);
	if(ret != 0)
	{
		return false;
	}
...
bool difficultDoeth::minimizePromoted(const char* ip, unsigned short port)
{
...
int difficultDoeth::treatedArraytheEnumerator(const char* host, unsigned short port){
	NETLog("--------------------------------------------------------------");
	NETLog("[difficultDoeth::treatedArraytheEnumerator] connect start %s : %d", host, port);
	...
	{
		NETLog("[difficultDoeth::treatedArraytheEnumerator] getaddrinfo error for %s: %s", host, gai_strerror(n));
		return 2;
	}
	if (res == nullptr)
	{
		NETLog("[difficultDoeth::treatedArraytheEnumerator] getaddrinfo error 2 for %s", host);
		return 2;
	}
	ressave = res;
	do 
	{
		...
		if (m_sock < 0)
		{
			NETLog("[difficultDoeth::treatedArraytheEnumerator] socket error: %d, %d, %d", res->ai_family, res->ai_socktype, res->ai_protocol);
			continue;
		}
		NETLog("[difficultDoeth::treatedArraytheEnumerator] socket ok: %d, %d, %d", res->ai_family, res->ai_socktype, res->ai_protocol);
		...
		if (res->ai_family == AF_INET)
		{
			...
			if (inet_ntop(res->ai_family, &sin->sin_addr, buf, sizeof(buf)) != nullptr)
				NETLog("[difficultDoeth::treatedArraytheEnumerator] connect start %s : %d", buf, ntohs(sin->sin_port));
#endif
		}
		else if (res->ai_family == AF_INET6)
		{
			...
			if (inet_ntop(res->ai_family, &sin->sin6_addr, buf, sizeof(buf)) != nullptr)
				NETLog("[difficultDoeth::treatedArraytheEnumerator] connect start %s : %d", buf, ntohs(sin->sin6_port));
#endif
		}
		else
		{
	...
	if (res == nullptr)
	{
		...
		m_sock = INVALID_SOCKET;
		NETLog("[difficultDoeth::treatedArraytheEnumerator] socket connect error : addr");
		NETLog("--------------------------------------------------------------");
		return 3;
	}
	freeaddrinfo(ressave);
	...
	{
		NETLog("[difficultDoeth::treatedArraytheEnumerator] socket connect error");
		NETLog("--------------------------------------------------------------");
		return 3;
	}
	NETLog("--------------------------------------------------------------");
...
int difficultDoeth::Send(const char* buf, int len, int flags)
{
	int bytes;
	int count = 0;
	while ( count < len ) {
...
int difficultDoeth::Recv(char* buf , int len , int flags)
{
