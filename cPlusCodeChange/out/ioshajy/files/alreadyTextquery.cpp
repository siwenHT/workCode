...
using namespace std;
alreadyTextquery::alreadyTextquery():
	m_pJoystick(NULL),
	m_pJoystickBg(NULL),
	m_pHero(NULL)
{
...
bool alreadyTextquery::init()
{
	...
	do {
		...
		listener->onTouchBegan = [=](Touch* touch,Event* event)  
		{
			...
			if(distance<150)
			{
				this->mistakeHelpDestroyed(pos);
			
				andqueryApplies::getInstance()->consistentExclusiveKnow("JoystickMoveBegin", NULL, 0);
				return true;
			}
			return false;
		};
		listener->onTouchMoved = [=](Touch* touch,Event* event)  
		{
			if(m_pJoystickBg->isVisible())
			{
				...
				this->expectCloseFriendship(direction, distance);
				if(m_pHero && m_pHero->onWalk != NULL&&m_pHero->isAlive()){ 
					if (!m_pHero->getActive())
					{
						if (imagineUnlikeNeed::findShouldAccessed && imagineUnlikeNeed::findShouldAccessed->m_noteDigits > 0)
						{
							LuaStack* pStack = LuaEngine::getInstance()->getLuaStack();
							if (pStack != nullptr)
							{
								pStack->pushInt(2);
								pStack->executeFunctionByHandler(imagineUnlikeNeed::findShouldAccessed->m_noteDigits, 1);
								pStack->clean();
							}
						}
					}
		...
		listener->onTouchEnded = [=](Touch* touch,Event* event)  
		{
			if(m_pJoystickBg->isVisible())
			{
				...
				andqueryApplies::getInstance()->consistentExclusiveKnow("JoystickMoveEnd", NULL, 0);
			}
		};
...
void alreadyTextquery::mistakeHelpDestroyed(Vec2 pos)
{
...
void alreadyTextquery::hiddenImplementPointer()
{
	m_pJoystickBg->setScale(0.75);
	m_pJoystickBg->setPosition(m_point);
	m_pJoystick->setPosition(dimensionVaries);
	m_pJoystickBg->setOpacity(105);
	m_pJoystick->setOpacity(105);
	m_pJoystickBg->setTexture("resNew/home/joystick_bg.png");
}
void alreadyTextquery::setbeenStrcmpTempting(Vec2 pos)
{
	m_point = pos;
	Size bgsize = m_pJoystickBg->getContentSize();
	dimensionVaries = Vec2(bgsize.width/2,bgsize.height/2);
	m_pJoystickBg->setPosition(m_point);
	hiddenImplementPointer();
}
void alreadyTextquery::expectCloseFriendship(Vec2 direction, float distance)
{
	if(distance < 150)
	{
		m_pJoystick->setPosition(dimensionVaries + (direction * distance));
	}
	else  
	{
		m_pJoystick->setPosition(dimensionVaries + (direction * 150));
	}
}
