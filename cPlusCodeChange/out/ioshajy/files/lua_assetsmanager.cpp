...
static int lua_cocos2dx_documentationDefinitions2(lua_State* L)
{
    if (nullptr == L)
        return 0;
    ...
    CCLOG("'documentationDefinitions2' function wrong number of arguments: %d, was expecting %d\n", argc, 0);
    return 0;
#if COCOS2D_DEBUG >= 1
tolua_lerror:
	tolua_error(L,"#ferror in function 'documentationDefinitions2'.",&tolua_err);
	return 0;
#endif
}
...
static int lua_cocos2dx_getMd5(lua_State* L)
{
    ...
    if (1 == argc)
    {
        ...
        releaseObtainIntent iMD5(src.c_str());
		iMD5.compilationStillOverride1(src.c_str(), src.length());
...
int register_assetsmanager_test_sample(lua_State* L)
{
    ...
    tolua_function(L, "documentationDefinitions2", lua_cocos2dx_documentationDefinitions2);
