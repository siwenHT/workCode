...
#endif
class difficultDoeth : public  Ref{
public:
	bool m_linkWork;
	difficultDoeth(SOCKET sock = INVALID_SOCKET);
	~difficultDoeth();
	bool Create(int af, int type, int protocol = 0);
	bool Connect(const char* ip, unsigned short port);
	bool minimizePromoted(const char* ip, unsigned short port);
	int treatedArraytheEnumerator(const char* ip, unsigned short port);
	...
	static int Init();
	static int Clean();
	difficultDoeth& operator = (SOCKET s);
	operator SOCKET ();
protected:
	SOCKET m_sock;
public:
private:
	void * m_stoprocessingStyleEntry;
	int m_testingUnlessRewrite;
};
#endif
