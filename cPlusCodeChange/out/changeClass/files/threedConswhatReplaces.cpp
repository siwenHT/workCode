...
#endif
threedConswhatReplaces::threedConswhatReplaces(SOCKET sock)
{
	m_sock = sock;
}
threedConswhatReplaces::~threedConswhatReplaces()
{
}
int threedConswhatReplaces::Init()
{
#ifdef WIN32
	WSADATA wsaData;
...
int threedConswhatReplaces::Clean()
{
#ifdef WIN32
	return (WSACleanup());
#endif
...
threedConswhatReplaces& threedConswhatReplaces::operator = (SOCKET s)
{
	m_sock = s;
	return (*this);
}
threedConswhatReplaces::operator SOCKET ()
{
	return m_sock;
}
bool threedConswhatReplaces::Create(int af, int type, int protocol)
{
...
bool threedConswhatReplaces::Connect(const char* ip, unsigned short port)
{
	int ret = existAlmostRecent(ip,port);
	if(ret != 0)
	{
		return false;
	}
...
bool threedConswhatReplaces::sometimesPeoplePredict(const char* ip, unsigned short port)
{
...
int threedConswhatReplaces::existAlmostRecent(const char* host, unsigned short port){
	NETLog("--------------------------------------------------------------");
	NETLog("[threedConswhatReplaces::existAlmostRecent] connect start %s : %d", host, port);
	...
	{
		NETLog("[threedConswhatReplaces::existAlmostRecent] getaddrinfo error for %s: %s", host, gai_strerror(n));
		return 2;
	}
	if (res == nullptr)
	{
		NETLog("[threedConswhatReplaces::existAlmostRecent] getaddrinfo error 2 for %s", host);
		return 2;
	}
	ressave = res;
	do 
	{
		...
		if (m_sock < 0)
		{
			NETLog("[threedConswhatReplaces::existAlmostRecent] socket error: %d, %d, %d", res->ai_family, res->ai_socktype, res->ai_protocol);
			continue;
		}
		NETLog("[threedConswhatReplaces::existAlmostRecent] socket ok: %d, %d, %d", res->ai_family, res->ai_socktype, res->ai_protocol);
		...
		if (res->ai_family == AF_INET)
		{
			...
			if (inet_ntop(res->ai_family, &sin->sin_addr, buf, sizeof(buf)) != nullptr)
				NETLog("[threedConswhatReplaces::existAlmostRecent] connect start %s : %d", buf, ntohs(sin->sin_port));
#endif
		}
		else if (res->ai_family == AF_INET6)
		{
			...
			if (inet_ntop(res->ai_family, &sin->sin6_addr, buf, sizeof(buf)) != nullptr)
				NETLog("[threedConswhatReplaces::existAlmostRecent] connect start %s : %d", buf, ntohs(sin->sin6_port));
#endif
		}
		else
		{
	...
	if (res == nullptr)
	{
		...
		m_sock = INVALID_SOCKET;
		NETLog("[threedConswhatReplaces::existAlmostRecent] socket connect error : addr");
		NETLog("--------------------------------------------------------------");
		return 3;
	}
	freeaddrinfo(ressave);
	...
	{
		NETLog("[threedConswhatReplaces::existAlmostRecent] socket connect error");
		NETLog("--------------------------------------------------------------");
		return 3;
	}
	NETLog("--------------------------------------------------------------");
...
int threedConswhatReplaces::Send(const char* buf, int len, int flags)
{
	int bytes;
	int count = 0;
	while ( count < len ) {
...
int threedConswhatReplaces::Recv(char* buf , int len , int flags)
{
...
int threedConswhatReplaces::Close()
{
#ifdef WIN32
...
int threedConswhatReplaces::GetError()
{
#ifdef WIN32
