...
bool AppDelegate::applicationDidFinishLaunching()
{
	...
	illustrateDiscussion::setenumeratorsContaiany(1.05);
	...
	director->_onExitCallback = CC_CALLBACK_0(AppDelegate::usuallyAbstractOmit, this);
	if (m_fpsLogEnabled )
	{
		m_bLogThreadNeedQuit = false;
		...
		if (m_Log.is_open())
		{
			director->onUpdate = CC_CALLBACK_1(AppDelegate::onalthoughAppearsMarker, this);
			m_pLogThread = new std::thread(&AppDelegate::expressionasWhose, this);
		}
	}
...
void AppDelegate::applicationDidEnterBackground()
{
    ...
    SimpleAudioEngine::getInstance()->pauseAllintroducedBookstore();
	startingCompile *luaEventMgr = startingCompile::instance();
	implementContainingMeans buff = luaEventMgr->getlargestWorksStores(11001);
	intendedForwardingMust::getInstance()->intvaluesIteratorStyle(buff);
...
void AppDelegate::applicationWillEnterForeground()
{
    ...
    SimpleAudioEngine::getInstance()->resumeAllintroducedBookstore();
	Director::getInstance()->getEventDispatcher()->dispatchCustomEvent("BACK_FOREGROUND");
}
void AppDelegate::usuallyAbstractOmit()
{
	m_bLogThreadNeedQuit = true;
	m_minimizeStore.notify_one();
	if (m_pLogThread)
	{
...
void AppDelegate::onalthoughAppearsMarker(float dt)
{
	m_accumDt += dt;
	if (m_accumDt >= 1.0f)
	{
...
void AppDelegate::expressionasWhose()
{
	std::mutex signalMutex;
	std::unique_lock<std::mutex> signal(signalMutex);
	std::vector<sequenceInstantion>	vFPSlogs;
	while (!m_bLogThreadNeedQuit)
	{
		...
		if (vFPSlogs.empty())
		{
			m_minimizeStore.wait(signal);
			continue;
		}
		for (unsigned int i = 0; i < vFPSlogs.size(); i++)
		{
			const sequenceInstantion& log = vFPSlogs[i];
