...
YGirdViewCell *YGirdView::dequeueCell()
{
    ...
    else
    {
		...
		// 取出来的cell有可能 _transformUpdated 为false ，导致cell的Node::visit的_modelViewTransform不会刷新
...
void YGirdView::reloadData()
{
    ...
    // if (_dataSource->numberOfCellsInGirdView(this) > 0)
...
bool YGirdView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
    ...
    if(_touches.size() == 1)
    {
		...
		//Point p = _container->getPosition();
		//CCLOG("pos.x = %f, pos.y = %f", p.x, p.y);
		...
		{		
			...
			//Point p = _touchedCell->getPosition();
			//CCLOG("cell.x = %f, cell.y = %f", p.x, p.y);
		}
